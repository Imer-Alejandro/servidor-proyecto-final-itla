...
export const obtenerTodosDocentes = async () => {
  const [rows] = await pool.promise().query(
    `SELECT u.usuario_id, u.nombre, u.apellido, u.email, u.estado, d.especialidad, d.titulo_academico
     FROM Usuario u
     JOIN Perfil_Docente d ON u.usuario_id = d.docente_id
     WHERE u.rol = 'DOCENTE'`
  );
  return rows;
};
...
export const registrarAsistencia = async ({ inscripcion_id, seccion_id, fecha_clase, registrada_por, estado, comentario }) => {
  const [result] = await pool.promise().query(
    `INSERT INTO Asistencia (inscripcion_id, seccion_id, fecha_clase, registrada_por, estado, comentario)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [inscripcion_id, seccion_id, fecha_clase, registrada_por, estado, comentario]
  );
  return { asistencia_id: result.insertId, inscripcion_id, seccion_id };
};

// Registrar calificación
export const registrarCalificacion = async ({ evaluacion_id, inscripcion_id, nota_obtenida, comentario_docente, registrada_por }) => {
  const [result] = await pool.promise().query(
    `INSERT INTO Calificacion (evaluacion_id, inscripcion_id, nota_obtenida, comentario_docente, registrada_por)
     VALUES (?, ?, ?, ?, ?)`,
    [evaluacion_id, inscripcion_id, nota_obtenida, comentario_docente, registrada_por]
  );
...
export const obtenerEvaluacionesSeccion = async (seccion_id) => {
  const [rows] = await pool.promise().query(
    `SELECT * FROM Evaluacion WHERE seccion_id = ?`,
    [seccion_id]
  );
  return rows;
};
